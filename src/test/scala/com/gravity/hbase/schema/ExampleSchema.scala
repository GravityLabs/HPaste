...
enable 'schema_example'"""
  ...
  @Test def testWithKeys() {
    withCleanup {
      ...
      println("ASYNC TIME")
      val robbie = ExampleSchema.ExampleTable.query2.withKey("Robbie").withFamilies(_.meta).singleOptionAsync().get
      robbie.prettyPrint()

      Assert.assertEquals(bros("Robbie").prettyFormat(), robbie.prettyFormat())
    }
  }
